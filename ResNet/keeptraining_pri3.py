...
class LabelGenerator(nn.Module):
    def __init__(self, psi):
        ...
        # self.classifier1 = nn.Sequential(
            # nn.Linear(filter[-3], filter[-2]),  #256->128
            # nn.ReLU(inplace=True),
            # nn.Linear(filter[-2],filter[-1]),   #128->64
            # nn.ReLU(inplace=True),
            # nn.Linear(filter[-1], len(psi)),    #64-> primiary task num
            # nn.Softmax(dim=1)
        # )
...
with open("log.txt", "w") as f:
    for index in range(pre_epoch,total_epoch):
        ...
        for i in range(train_batch):
            ...
            train_loss2 = Res_model.model_fit(train_pred2, train_pred3, pri=False, num_output=30)
